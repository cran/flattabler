---
title: "flattabler: Obtaining a Flat Table from Pivot Tables in R"
author: "Jose Samos (jsamos@ugr.es)"
date: "2020-06-20"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{flattabler: Obtaining a Flat Table from Pivot Tables in R}
...
The transformation to obtain the flat table from the pivot table has been carried out in R, using `flattabler` package. The code is shown below. For clarity, the pipe operator syntax provided by `tidyr` package has been used.
...
1. __Pivot table import__: Import pivot tables into an object or list of objects. We start from data in text files, or previously imported data in a data frame, and generate pivot table objects from them.
...
Two formats have been considered: text file, and data frame.

In the case of working with files, the situation of jointly treating all the files in a folder has also been considered. 
